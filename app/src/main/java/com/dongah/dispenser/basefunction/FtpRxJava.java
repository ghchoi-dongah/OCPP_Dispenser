...
//    private static String REMOTE_PATH = "/app/upload/dongah/apk";
    ...
    public FtpRxJava(FileTransType fileTransType, String location) {
        ...
        REMOTE_PATH = lastSlash > 0 ? fullPath.substring(0, lastSlash) : "/"; // "/dir"
        String urlFileName = fullPath.substring(lastSlash + 1);               // "filename.apk"

        if (fileTransType == FileTransType.FIRMWARE || fileTransType == FileTransType.SIGNED_FIRMWARE) {
            UI_FILE_NAME = urlFileName;
            GlobalVariables.setFirmwareFileName(UI_FILE_NAME);
        } else if (fileTransType == FileTransType.DIAGNOSTICS) {
            UI_FILE_NAME = "diagnostics";
        } else {
            UI_FILE_NAME = "securityLog.dongah";
        }
    }
