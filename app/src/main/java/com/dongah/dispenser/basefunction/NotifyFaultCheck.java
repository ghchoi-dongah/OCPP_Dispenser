...
public class NotifyFaultCheck {
    ...
    public void initDisconnectState(boolean faulted) {
        UiDSP.setResultCompare(faulted);
    }
        ...
        try {
            ...
            if (!Objects.equals(UiDSP.ResultCompare, disconnected)) {
                ...
                if (disconnected) {
                    ...
                    // Faulted 상태였을 때만 Available로 복귀 — Preparing/Charging 등 활성 상태는 덮어쓰지 않음
                    if (Objects.equals(chargingCurrentData.getChargePointStatus(), ChargePointStatus.Faulted)) {
            ...
            if (!Objects.equals(csUnPlug.ResultCompare, rxData.csPilot)) {
                ...
                if (!rxData.csPilot) {
                    ...
                    if (isPlugStatus && Objects.equals(authMode, "0")) {
                        new Handler(Looper.getMainLooper()).postDelayed(() -> {
                            // RemoteStart 진행 중이면 Available 전송 안함 — 이전 테스트의 지연 타이머가 다음 TC의 Preparing을 덮어쓰는 경쟁 조건 방지
                            if (!chargingCurrentData.isRemoteStart()) {
                                chargingCurrentData.setChargePointErrorCode(ChargePointErrorCode.NoError);
                                chargingCurrentData.setChargePointStatus(ChargePointStatus.Available);
                                processHandler.sendMessage(socketReceiveMessage.onMakeHandlerMessage(
                                        GlobalVariables.MESSAGE_HANDLER_STATUS_NOTIFICATION,
