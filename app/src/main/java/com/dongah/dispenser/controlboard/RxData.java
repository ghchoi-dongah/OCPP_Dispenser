...
public class RxData {
    ...
    // 402 address
    public short cpVoltage = 120;           // cp 전압 (ex) 1198 => 119.8 / 초기값 120 = 12.0V(케이블 미연결)
    // 403 address
...
//            csFault = csPLCComm || csPowerMeterComm;
            ...
            reserved0 = data[1];
            cpVoltage = data[2];
            // CP 전압 기반 케이블 연결 감지: 11V(raw 110) 이하이면 케이블 연결됨
            // 2번 연속인 경우 csPilot 변경
            boolean below = ((cpVoltage * 0.1) < 11.0);
            csPilot = (below && prevBelow);
            prevBelow = below;
